        ...
        for entry in fs::read_dir(text_dir).unwrap() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("txt") {
                ...
                match shared::DataLangFile::parse_from_str(&content) {
                    Ok(parsed) => {
                        if let Err(validation_error) = parsed.validate() {
                            panic!(
                                "Validation failed for {}.txt: {}",
                                file_stem, validation_error
                            );
                        }
                ...
                let rust_content = [
                    ...
                    &format!(
                        "// 1. Edit {}.txt in the text_definitions/ directory",
                        file_stem
                    ),
                ...
                let output_path =
                    Path::new(macro_definitions_dir).join(format!("{}.rs", file_stem));
