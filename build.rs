    ...
    if text_dir.exists() && text_dir.is_dir() {
        for entry in fs::read_dir(text_dir).unwrap() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("txt") {
                ...
                match shared::DataLangFile::parse_from_str(&content) {
                    Ok(parsed) => {
                        if let Err(validation_error) = parsed.validate() {
                            panic!(
                                "Validation failed for {file_stem}.txt: {validation_error}"
                            );
                        }
                ...
                let rust_content = [
                    &format!(
                        "// This file is automatically generated from {file_stem}.txt"
                    ),
                    "// DO NOT EDIT MANUALLY - your changes will be overwritten",
                    "//",
                    "// To regenerate this file:",
                    &format!(
                        "// 1. Edit {file_stem}.txt in the text_definitions/ directory"
                    ),
                ...
                let output_path =
                    Path::new(macro_definitions_dir).join(format!("{file_stem}.rs"));
